// src/pages/patient/Sessions.jsx
import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link } from 'react-router-dom';
import { Calendar, Video, Clock, X, FileText, Loader2 } from 'lucide-react';
...
const PatientSessions = () => {
                ...
                try {
                    ...
                    (histRes.data.sessions || []).forEach(s => {
                        if (s.appointment?._id) {
                            historyMap[s.appointment._id] = { sessionNote: s.sessionNote, questionnaireResponses: s.questionnaireResponses };
                        }
                    });
                } catch (_) { }

                const merged = allApts.filter(apt => apt.paymentStatus === 'paid').map(apt => ({
                    ...apt,
    ...
    const sortedAppointments = [...appointments].map(apt => {
        ...
        if (timeMatch) { tH = parseInt(timeMatch, 10); tM = parseInt(timeMatch || '0', 10); }
        ...
        if (!isValidDate) displayStatus = 'Unknown Date';
        else if (apt.status === 'cancelled') displayStatus = 'Cancelled';
        else if (isPast) displayStatus = 'Ended';
        else if (isOngoing) displayStatus = 'Ongoing';
        else if (diff > 0 && diff <= 60) displayStatus = `In ${Math.ceil(diff)}m`;
        else if (diff > 60) {
            const days = Math.floor(diff / 1440);
            const hours = Math.floor((diff % 1440) / 60);
            if (days > 0) displayStatus = `In ${days}d`; else displayStatus = `In ${hours}h`;
        }
        ...
        <div key={apt._id} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-4 flex-1">
                <div className="w-12 h-12 rounded-2xl bg-primary-light flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {apt.doctor?.profilePic ? <img src={apt.doctor.profilePic} alt={apt.doctor.name} className="w-full h-full object-cover" /> : <span className="text-2xl">👨‍⚕️</span>}
                </div>
                <div>
                    <p className="font-semibold text-text-primary">
                        {apt.doctor?.name ? (apt.doctor.name.toLowerCase().startsWith('dr') ? apt.doctor.name : `Dr. ${apt.doctor.name}`) : 'Dr. Therapist'}
                    </p>
            ...
            <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                ...
                {apt.isOngoing ? (
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleViewDetail(apt)} className="px-4 py-2 rounded-xl bg-primary-light text-primary text-sm font-medium hover:bg-primary/10 transition-colors flex items-center gap-1">
                    ...
                    <span className="text-sm font-medium px-4 py-2 rounded-xl bg-danger/10 text-danger">Cancelled</span>
                ) : (
                    <span className="text-sm font-medium px-3 py-1.5 rounded-xl bg-primary-light text-primary">{apt.displayStatus}</span>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-background">
                            ...
                            <>
                                {sortedAppointments.filter(a => a.isOngoing).length > 0 && (
                                    <div className="card border-l-4 border-l-amber-400">
                                        <h2 className="text-xl font-bold text-text-primary mb-4 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />Ongoing Session</h2>
                                        <div className="space-y-4">{sortedAppointments.filter(a => a.isOngoing).map(renderAppointment)}</div>
                                    </div>
                                )}
                                {sortedAppointments.filter(a => !a.isPast && !a.isOngoing).length > 0 && (
                                    <div className="card">
                                        <h2 className="text-xl font-bold text-text-primary mb-4 flex items-center gap-2"><Calendar className="w-5 h-5 text-primary" />Upcoming Sessions</h2>
                                        <div className="space-y-4">{sortedAppointments.filter(a => !a.isPast && !a.isOngoing).map(renderAppointment)}</div>
                                    </div>
                                )}
                                {sortedAppointments.filter(a => a.isPast).length > 0 && (
                                    <div className="card">
                                        <h2 className="text-xl font-bold text-text-primary mb-4">Past Sessions</h2>
                                        <div className="space-y-4">{sortedAppointments.filter(a => a.isPast).reverse().map(renderAppointment)}</div>
                                    </div>
                                )}
                            </>
                        ) : (
            ...
            <AnimatePresence>
                {selectedSession && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setSelectedSession(null); setSessionDetail(null); }}>
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl z-10">
                                <div>
                                    <h3 className="font-display font-bold text-lg text-text-primary">Session Details</h3>
                                    <p className="text-sm text-text-secondary">{selectedSession.isValidDate ? selectedSession.safeDate.toLocaleDateString('en', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'Unknown Date'} • {selectedSession.time}</p>
                                </div>
                                <button onClick={() => { setSelectedSession(null); setSessionDetail(null); }} className="p-2 rounded-xl hover:bg-gray-100 text-text-secondary transition-colors"><X className="w-5 h-5" /></button>
                            </div>
                            ...
                            <div className="p-5 space-y-5">
                                {detailLoading ? (
                                    ...
                                    <>
                                        ...
                                        <div>
                                            <h4 className="font-semibold text-text-primary mb-3 flex items-center gap-2"><FileText className="w-4 h-4 text-primary" />Doctor's Report</h4>
                                            {sessionDetail?.sessionNote?.report ? <div className="p-4 rounded-xl bg-green-50 border border-green-200"><p className="text-sm text-text-primary leading-relaxed">{sessionDetail.sessionNote.report}</p></div> : <p className="text-sm text-text-secondary italic">No report generated yet</p>}
                                        </div>
                                    </>
                                )}
                            </div>
